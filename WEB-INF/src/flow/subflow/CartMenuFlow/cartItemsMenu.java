...
public class cartItemsMenu extends com.avaya.sce.runtime.Menu {
	List<String> bookProducts= new ArrayList();
	 ...
	 * Last generated by Orchestration Designer at: 2023-FEB-09  01:21:38 PM
	...
	public java.util.Collection getEvents(com.avaya.sce.runtimecommon.SCESession mySession) {
		...
		event = new com.avaya.sce.runtime.Event(com.avaya.sce.runtimecommon.SCERT.EVENT_NOINPUT, (com.avaya.sce.runtime.PromptRefInfo[])eventPromptNames.toArray(new com.avaya.sce.runtime.PromptRefInfo[0]), new com.avaya.sce.runtime.Goto("CartMenuFlow-NoInputcartItems", 2, true, ""));
		list.add(event);
		...
		event = new com.avaya.sce.runtime.Event(com.avaya.sce.runtimecommon.SCERT.EVENT_NOMATCH, (com.avaya.sce.runtime.PromptRefInfo[])eventPromptNames.toArray(new com.avaya.sce.runtime.PromptRefInfo[0]), new com.avaya.sce.runtime.Goto("CartMenuFlow-NoMatchcartItems", 2, true, ""));
		list.add(event);
		return(list);
	}
	/**
	...
	public java.util.Collection getChoices(com.avaya.sce.runtimecommon.SCESession mySession) {
		...
		choice = new com.avaya.sce.runtime.Choice("Exit", "0", true, "exact", "CartMenuFlow-cartProductCount", grammarInfo, true);
		list.add(choice);
	...
	public void updatePrompts(Collection prompts, SCESession mySession) {
		...
		try {
			...
			int pageCount =0;
			if (bookProducts.size() % 6 == 0) {
				pageCount = bookProducts.size() / 6;
			} else {
				pageCount = (bookProducts.size() / 6) + 1;
			}
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_INFO, "CategoryMenuPageNumber :"+CategoryMenuPageNumber+"\npageCount :"+pageCount+"\nCategorySize :"+bookProducts.size(), mySession);
			...
			if (pageCount==1) {
				for (int i = 0; i < bookProducts.size(); i++) {
					File url=new File("http://172.16.11.17:8090/AudioPrompts/Elibrary/"+bookProducts.get(i).replaceAll(" ", "")+".wav");
					TraceInfo.trace(ITraceInfo.TRACE_LEVEL_INFO,url.toString(), mySession);
					em.setValue(url);  
				...
				if (CategoryMenuPageNumber == 0) {
					for (int i = 0; i < 6; i++) {
						p.add(1, new com.avaya.sce.runtime.PhraseVariableElement(
								...
								com.avaya.sce.runtime.PhraseVariableElement.Type.AUDIO_URL, false));
						p.add(1, new com.avaya.sce.runtime.PhraseVariableElement(
								"AudioPath:" + bookProducts.get(i).replaceAll(" ", ""),
								com.avaya.sce.runtime.PhraseVariableElement.Type.AUDIO_URL, false));
					}
					p.add(1, new com.avaya.sce.runtime.PhraseVariableElement(
					...
					for (int i = CategoryMenuPageNumber*6; i < bookProducts.size(); i++) {
						p.add(1, new com.avaya.sce.runtime.PhraseVariableElement(
	...
	public void requestBegin(SCESession mySession) {
		try {
			String userId = mySession.getVariableField(IProjectVariables.USER_DETAILS,
					IProjectVariables.USER_DETAILS_FIELD_USER_ID_NUMBER).getStringValue();

			IntegrationClass integration = new IntegrationClass();
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_INFO, "http://172.16.11.17:8082/cart/cartview"+userId, mySession);
			List<JSONObject> apiGet = integration.apiGet("http://172.16.11.17:8082/cart/cartview"+userId);
			for (int i = 0; i < apiGet.size(); i++) {
				bookProducts.add(apiGet.get(i).get("bookName").toString());
			...
			categoryList.setValue(bookProducts);
