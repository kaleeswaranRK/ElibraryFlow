 ...
 * Last generated by Orchestration Designer at: 2023-FEB-01  05:50:05 PM
 */
public class cartItemsMenu extends com.avaya.sce.runtime.Menu {
	List<String> booksList =new LinkedList();
	List<String> priceList =new LinkedList();
	List<String> quantityList =new LinkedList();
	//{{START:CLASS:FIELDS
	//}}END:CLASS:FIELDS
	...
	public java.util.Collection getEvents(com.avaya.sce.runtimecommon.SCESession mySession) {
		...
		event = new com.avaya.sce.runtime.Event(com.avaya.sce.runtimecommon.SCERT.EVENT_NOINPUT, (com.avaya.sce.runtime.PromptRefInfo[])eventPromptNames.toArray(new com.avaya.sce.runtime.PromptRefInfo[0]), new com.avaya.sce.runtime.Goto("CartMenuFlow-NoInputcartItems", 3, true, ""));
		list.add(event);
		...
		event = new com.avaya.sce.runtime.Event(com.avaya.sce.runtimecommon.SCERT.EVENT_NOMATCH, (com.avaya.sce.runtime.PromptRefInfo[])eventPromptNames.toArray(new com.avaya.sce.runtime.PromptRefInfo[0]), new com.avaya.sce.runtime.Goto("CartMenuFlow-NoMatchcartItems", 3, true, ""));
		list.add(event);
		return(list);
	}
	/**
	...
	public java.util.Collection getChoices(com.avaya.sce.runtimecommon.SCESession mySession) {
		...
		choice = new com.avaya.sce.runtime.Choice("Exit", "0", true, "exact", "CartMenuFlow-cartItemsMenuData", grammarInfo, true);
		list.add(choice);
	...
	public void updateChoices(Collection choices, SCESession mySession) {
		try {
			...
			int PageNumber = mySession
					...
					.getIntValue();
			int pageCount = 0;
			if (booksList.size() % 6 == 0) {
				pageCount = booksList.size() / 6;
			} else {
				pageCount = (booksList.size() / 6) + 1;
			}
			if (pageCount == 1) {
				for (int i = 0; i < booksList.size(); i++) {
					choice = new com.avaya.sce.runtime.Choice(booksList.get(i), String.valueOf(i + 1), true, "exact",
							"CartMenuFlow-cartItemsMenuData", grammarInfo, true);
					choices.add(choice);
				}
				choice = new com.avaya.sce.runtime.Choice("Repeat", "9", true, "exact", "CartMenuFlow-cartItemsMenu",
						grammarInfo, true);
				choices.add(choice);
			} else {
				if (PageNumber == 0) {
					for (int i = 0; i < 6; i++) {
						choice = new com.avaya.sce.runtime.Choice(booksList.get(i), String.valueOf(i + 1), true,
								"exact", "CartMenuFlow-cartItemsMenuData", grammarInfo, true);
						choices.add(choice);
					}
					choice = new com.avaya.sce.runtime.Choice("next", "7", true, "exact", "CartMenuFlow-cartItemsMenuNext",
							grammarInfo, true);
					choices.add(choice);
					choice = new com.avaya.sce.runtime.Choice("Repeat", "9", true, "exact", "CartMenuFlow-cartItemsMenu",
							grammarInfo, true);
					...
					for (int i = PageNumber * 6; i < booksList.size(); i++) {
						choice = new com.avaya.sce.runtime.Choice(booksList.get(i),
								String.valueOf(i - ((PageNumber * 6) - 1)), true, "exact",
								"CartMenuFlow-cartItemsMenuData", grammarInfo, true);
						choices.add(choice);
					}
					choice = new com.avaya.sce.runtime.Choice("Repeat", "9", true, "exact", "CartMenuFlow-cartItemsMenu",
					...
					choice = new com.avaya.sce.runtime.Choice("previous", "8", true, "exact",
							"CartMenuFlow-cartItemsMenuNext", grammarInfo, true);
					choices.add(choice);
				} else {
			...
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_ERROR, "Choices : " + choices.size(), mySession);
		} catch (Exception e) {
	...
	public void updatePrompts(Collection prompts, SCESession mySession) {
		try {
			...
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_INFO, "booksMenuPageNumber :" + MenuPageNumber + "\npageCount :"
					+ pageCount + "\nbookSize :" + booksList.size(), mySession);
			p.add(1, new com.avaya.sce.runtime.PhraseVariableElement("AudioPath:cartItemsMenu",
					com.avaya.sce.runtime.PhraseVariableElement.Type.AUDIO_URL, false));
			if (pageCount == 1) {
				for (int i = 0; i < booksList.size(); i++) {
					setValue.projectVariableSetValue(mySession, "http://172.16.11.43:8090/Audiopromts/Elibrary/"
							+ booksList.get(i).replaceAll(" ", "") + ".wav", i + 1);
					setValue.priceSetValue(mySession, priceList.get(i), i+1);
					setValue.quantitySetValue(mySession, quantityList.get(i), i+1);
				...
				if (MenuPageNumber == 0) {
					...
					for (int i = MenuPageNumber * 6; i < booksList.size(); i++) {
						setValue.projectVariableSetValue(mySession, "http://172.16.11.43:8090/Audiopromts/Elibrary/"
								+ booksList.get(i).replaceAll(" ", "") + ".wav", (i - ((MenuPageNumber * 6) - 1)));
						setValue.priceSetValue(mySession, priceList.get(i), (i - ((MenuPageNumber * 6) - 1)));
						setValue.quantitySetValue(mySession, quantityList.get(i), (i - ((MenuPageNumber * 6) - 1)));
						p.add(1, new com.avaya.sce.runtime.PhraseVariableElement("menuCommon:menuOption" + (i - ((MenuPageNumber * 6) - 1)),
								com.avaya.sce.runtime.PhraseVariableElement.Type.AUDIO_URL, false));
	...
	public void requestBegin(SCESession mySession) {
		try {
			mySession.getVariableField(IProjectVariables.DATA_ELIBRARY,
					IProjectVariables.DATA_ELIBRARY_FIELD_EXIT_LOCATION).setValue("cart Items Menu");
			booksList =(LinkedList<String>) mySession.getVariableField(IProjectVariables.MENU_COMMON,
					IProjectVariables.MENU_COMMON_FIELD_BOOK_MENU_LIST).getObjectValue();
			priceList =(LinkedList<String>) mySession.getVariableField(IProjectVariables.MENU_COMMON,
					IProjectVariables.MENU_COMMON_FIELD_PRICE_LIST).getObjectValue();
			quantityList =(LinkedList<String>) mySession.getVariableField(IProjectVariables.MENU_COMMON,
					IProjectVariables.MENU_COMMON_FIELD_QUANTITY_LIST).getObjectValue();
		} catch (Exception e) {
			TraceInfo.trace(ITraceInfo.TRACE_LEVEL_ERROR, e, mySession);
		}
	}
